...
function cleanSlot(v){
 ...
 const enemies=Array.isArray(s.enemies)?s.enemies.filter(e=>e&&Number.isInteger(e.id)&&e.id>=0&&e.id<=30).map(e=>({id:e.id,alive:e.alive!==false,hp:number(e.hp,0,e.id===5?280:e.id<5?64:600,e.id===5?280:e.id<5?64:600),x:number(e.x,-105,105,0),z:number(e.z,-105,105,0)})):null;
 return {id:typeof v.id==='string'?v.id:'character-'+Date.now(),name,classId:v.classId,createdAt:number(v.createdAt,0,1e15,Date.now()),updatedAt:number(v.updatedAt,0,1e15,Date.now()),state:{quests:Object.fromEntries(['yun','lin','mei','heart'].map(k=>[k,['active','done'].includes(s.quests?.[k])?s.quests[k]:'new'])),crystals:Array.isArray(s.crystals)?[...new Set(s.crystals.filter(n=>Number.isInteger(n)&&n>=0&&n<3))]:[],level,xp:Math.floor(number(s.xp,0,100+(level-1)*50-1,0)),hp:number(s.hp,1,100,100),mp:number(s.mp,0,100,100),position:{x:number(s.position?.x,-105,105,0),z:number(s.position?.z,-105,105,8)},rotation:number(s.rotation,-1000,1000,Math.PI),cooldowns:[0,1,2].map(i=>number(s.cooldowns?.[i],0,15,0)),shieldTime:number(s.shieldTime,0,5,0),enemies,camDistance:number(s.camDistance,4,44,30),camYaw:number(s.camYaw,-1e8,1e8,.48),camPitch:number(s.camPitch,.06,1.25,.6)}};
}
